...
def my_save(sp, hp, burned_draws, draws, fig):
    param_hash = get_hash_str(sp)
    print(param_hash, '\t', sp)
    dir_name = os.path.join(
        hp.save_dir, 
        f'PDB_{hp.model_num:02d}',
        f"drghmc__globalseed{hp.global_seed}_burnin{hp.burn_in}_chainlen{hp.chain_length}",
        f"paramhash_{param_hash}",
